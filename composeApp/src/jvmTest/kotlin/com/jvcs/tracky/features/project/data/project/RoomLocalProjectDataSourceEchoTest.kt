...
internal class RoomLocalProjectDataSourceEchoTest {
    ...
    private suspend fun seed(intervalEnd: Long? = null, device: String? = null) {
        ...
        db.projectDao.upsertTaskInterval(
            TaskIntervalEntity(
                ...
                startDateTimeEpochMs = 0, endDateTimeEpochMs = intervalEnd,
                durationMillis = intervalEnd ?: 0, startedByDeviceId = device
            )
        )
    }

    private fun echoed(end: Long?, device: String?) = TaskInterval(
        intervalId = "i1",
        parentTaskId = "t1",
        parentProjectId = "p1",
    ...
    @Test
    fun closesAnIntervalThisDeviceStillHasOpen() = runBlocking {
    ...
    @Test
    fun doesNotReopenAnIntervalThisDeviceAlreadyClosed() = runBlocking {
    ...
    @Test
    fun keepsTheProvenanceOfARowTheServerHasNoDeviceIdFor() = runBlocking {
    ...
    @Test
    fun neverDeletesARowTheEchoDidNotMention() = runBlocking {
    ...
    @Test
    fun skipsARowWhoseParentThisDeviceDoesNotHave() = runBlocking {
        ...
        dataSource.applyTimerEcho(
            taskIntervals = listOf(echoed(end = 60_000, device = null).copy(
                intervalId = "i-orphan",
