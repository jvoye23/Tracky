...
internal class ReorderTasksTest {
    ...
    @Test
    fun reorderTasks_writesOnlyTheIdsWhoseIndexActuallyMoved() = runBlocking<Unit> {
    ...
    @Test
    fun reorderTasks_skipsTheTasksThatDidNotMove() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b", "c"))
        f.taskRepository.reorderTasks("p1", listOf("a", "b", "c"))
        f.localTask.sortIndexWrites.clear()
    ...
    @Test
    fun reorderTasks_isANoOpWhenNothingMoved() = runBlocking<Unit> {
    ...
    @Test
    fun reorderTasks_ignoresIdsThatAreNotInTheProject() = runBlocking<Unit> {
    ...
    @Test
    fun reorderTasks_pushesTheWholeGestureAsOneRequest() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b", "c"))
    ...
    @Test
    fun reorderTasks_stampsOneTimestampFromTheInjectedClock() = runBlocking<Unit> {
    ...
    @Test
    fun reorderTasks_doesNotCallTheServerWhenTheLocalWriteFails() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b"))
    ...
    @Test
    fun reorderTasks_queuesOneMarkerWhenOffline() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b"))
    ...
    @Test
    fun reorderTasks_collapsesRepeatOfflineReordersIntoOneQueueRow() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b", "c"))
        f.remoteTask.reorderFailWith = DataError.Remote.NO_INTERNET
    ...
    @Test
    fun reorderTasks_queueRowDoesNotCollideWithTheProjectsOwnPendingWrite() = runBlocking<Unit> {
        ...
        f.remoteTask.reorderFailWith = DataError.Remote.NO_INTERNET
        f.projectRepository.upsertProject(f.db.projects.getValue("p1").copy(title = "renamed"))
    ...
    @Test
    fun syncPendingTasks_rebuildsTheOrderFromLocalStateWhenTheMarkerDrains() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b", "c"))
        f.remoteTask.reorderFailWith = DataError.Remote.NO_INTERNET
        f.taskRepository.reorderTasks("p1", listOf("c", "b", "a"))
    ...
    @Test
    fun syncPendingTasks_dropsATaskDeletedWhileTheReorderWasQueued() = runBlocking<Unit> {
        val f = fixtureWithTasks(taskIds = arrayOf("a", "b", "c"))
        f.remoteTask.reorderFailWith = DataError.Remote.NO_INTERNET
        f.taskRepository.reorderTasks("p1", listOf("c", "b", "a"))
        f.db.tasks.remove("b")
