...
class PerDayUiMapperTest {
    ...
    fun `a busier day outside the ten-day window is not the busiest`() {
        ...
        val strip =
            project(
                task(
                    intervals =
                        listOf(interval("2026-08-29T09:00:00Z", minutes = 600)) +
    ...
    private fun String.plusDays(days: Int): String = Instant.parse(this).plus(days.days).toString()

    // --- builders ----------------------------------------------------------------------------

    private fun project(vararg tasks: ProjectTask) =
        Project(
            projectId = "project",
            title = "Project",
            description = null,
    ...
    private fun task(intervals: List<TaskInterval> = emptyList(), subTasks: List<ProjectSubTask>? = null) =
        ProjectTask(
    ...
    private fun subTask(intervals: List<SubTaskInterval>) =
        ProjectSubTask(
