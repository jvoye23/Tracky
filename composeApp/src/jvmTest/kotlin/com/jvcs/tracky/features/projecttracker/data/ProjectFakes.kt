...
class FakeDb {
    ...
    fun cascadeDeleteSubTask(subTaskId: String) {
        ...
        subTaskIntervals.values
            .filter { it.parentSubTaskId == subTaskId }
    ...
    fun cascadeDeleteTaskInterval(intervalId: String) {
        ...
        subTaskIntervals.values
            .filter { it.parentTaskIntervalId == intervalId }
            .map { it.subTaskIntervalId }
            .forEach { subTaskIntervals.remove(it) }
    }
    ...
    fun newTask(taskId: String, projectId: String) =
        ProjectTask(
    ...
    fun seedTask(taskId: String, projectId: String) = newTask(taskId, projectId).also { tasks[taskId] = it }
...
class FakeLocalProjectDataSource(private val db: FakeDb = FakeDb()) :
    ...
    override suspend fun upsertProjects(projects: List<Project>): EmptyResult<DataError.Local> {
        projects.forEach { incoming ->
            if (serverWinsOnPull(
                    this.projects[incoming.projectId]?.ownUpdatedAt?.toEpochMilliseconds(),
        ...
        incomingTasks.forEach { incoming ->
            if (serverWinsOnPull(
                    tasks[incoming.projectTaskId]?.ownUpdatedAt?.toEpochMilliseconds(),
                    incoming.ownUpdatedAt?.toEpochMilliseconds(),
                )
            ) {
        ...
        incomingTasks.flatMap { it.subTasks.orEmpty() }.forEach { incoming ->
            ...
            if (serverWinsOnPull(
                    subTasks[incoming.projectSubTaskId]?.ownUpdatedAt?.toEpochMilliseconds(),
                    incoming.ownUpdatedAt?.toEpochMilliseconds(),
                )
            ) {
        ...
        upsertProjects(
            ...
            changes.projects.map { project ->
                project.copy(
                    projectTasks =
                        changes.tasks
                            .filter { it.parentProjectId == project.projectId }
                            .map { task ->
                                task.copy(
                                    intervals = changes.taskIntervals.filter { it.parentTaskId == task.projectTaskId },
                                    subTasks =
                                        changes.subTasks
                                            .filter { it.parentProjectTaskId == task.projectTaskId }
                                            .map { sub ->
                                                sub.copy(
                                                    subTaskIntervals =
                                                        changes.subTaskIntervals
    ...
    override suspend fun applyTimerEcho(
        ...
        subTaskIntervals.forEach { incoming ->
            ...
            if (local == null ||
                ...
                db.subTaskIntervals[incoming.subTaskIntervalId] =
                    incoming.copy(
                        startedParentTimer = local?.startedParentTimer ?: false,
                        startedByDeviceId = incoming.startedByDeviceId ?: local?.startedByDeviceId,
                    )
...
class FakeLocalTaskDataSource(private val db: FakeDb = FakeDb()) : LocalTaskDataSource {
    ...
    override fun getTaskWithIntervalsById(taskId: String): Flow<ProjectTask?> = flowOf(tasks[taskId]?.withIntervals())
 ...
 */
internal class RepoFixture(val time: FakeTimeProvider = FakeTimeProvider()) {
    val db = FakeDb()
