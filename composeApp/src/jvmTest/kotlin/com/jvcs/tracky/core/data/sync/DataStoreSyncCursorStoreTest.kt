...
internal class DataStoreSyncCursorStoreTest {
    ...
    @Test
    fun aDeviceThatHasNeverPulledHasNoCursor() = runTest {
        assertNull(DataStoreSyncCursorStore(createTestDataStore()).cursor())
    }

    @Test
    fun theCursorRoundTrips() = runTest {
    ...
    @Test
    fun aFreshStoreReadsThePersistedCursorBack() = runTest {
    ...
    @Test
    fun theCursorNeverGoesBackwards() = runTest {
        val store = DataStoreSyncCursorStore(createTestDataStore())
        store.setCursor(84_213)
    ...
    @Test
    fun concurrentAdvancesSettleOnTheHighest() = runTest {
        val store = DataStoreSyncCursorStore(createTestDataStore())
    ...
    @Test
    fun aClearedStoreAcceptsALowerCursorAgain() = runTest {
        val store = DataStoreSyncCursorStore(createTestDataStore())
        store.setCursor(84_213)
        store.clear()
