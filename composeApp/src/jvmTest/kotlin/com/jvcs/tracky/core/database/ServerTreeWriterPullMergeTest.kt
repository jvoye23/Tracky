...
class ServerTreeWriterPullMergeTest {
    ...
    private fun projectEntity(id: String, updatedAt: Long?) =
        ProjectEntity(
            projectId = id,
            title = "title-$id",
            description = null,
    ...
    @Test
    fun takesTheServersDeviceIdForAnIntervalItHasNeverSeen() =
        runBlocking {
            seedTask()
    ...
    @Test
    fun skipsAnOrphanSubTaskInsteadOfLosingTheWholePull() =
        runBlocking<Unit> {
    ...
    @Test
    fun skipsASubTaskIntervalWithEitherParentMissing() =
        runBlocking<Unit> {
            seedSubTask()
